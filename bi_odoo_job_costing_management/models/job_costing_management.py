...
class JobCostSheet(models.Model):
    ...
    total_tools_cost = fields.Float(compute='_compute_total_tools_cost', string='Total tools Cost', default=0.0)
    total_machines_cost = fields.Float(compute='_compute_total_machines_cost', string='Total machines Cost',default=0.0)
    ...
    @api.multi
    def _compute_total_tools_cost(self):
        total = 0.0
        for line in self.tools_job_cost_line_ids:
            total += line.subtotal
        self.total_tools_cost = total

    @api.multi
    def _compute_total_machines_cost(self):
        total = 0.0
        for line in self.machines_job_cost_line_ids:
            total += line.subtotal
        self.total_machines_cost = total

    @api.multi
    def _compute_total_cost(self):
        total = 0.0
        total = self.total_material_cost + self.total_labour_cost + self.total_overhead_cost + self.total_tools_cost + self.total_machines_cost
...
class JobCostLine(models.Model):
    ...
    tools_job_cost_sheet_id = fields.Many2one('job.cost.sheet', 'Tools Job Cost Sheet')
    machines_job_cost_sheet_id = fields.Many2one('job.cost.sheet', 'Machines Job Cost Sheet')
    ...
    def _compute_purchase_quantity(self):
        for line in self:
            qty = 0
            purchase_order_line_ids = self.env['purchase.order.line'].search([('product_id', '=', line.product_id.id)])
            for purchase in purchase_order_line_ids:
    ...
    def _compute_invoice_quantity(self):
        for line in self:
            qty = 0
            invoice_line_ids = self.env['account.invoice.line'].search([('product_id', '=', line.product_id.id)])
...
class JobCostProduction(models.Model):
    _name = 'job.cost.report'

    sequence = fields.Char(string='Sequence', readonly=True, copy=False, index=True,
                           default=lambda self: self.env['ir.sequence'].get('job.cost.report'))
    project_id = fields.Many2one('project.project', string='Project')
    analytic_ids = fields.Many2one('account.analytic.account', string="Analytic Account")
    job_order_id = fields.Many2one('job.order', 'Job Order')
    job_issue_customer_id = fields.Many2one('res.partner', 'Job Issue Customer')
    ...
    close_date = fields.Datetime(string="Close Date", default=datetime.now())
    create_by_id = fields.Many2one('res.users', 'Created By')
    material_job_cost_line_ids = fields.One2many('job.cost.report.line', 'material_job_cost_report_id',
                                                 'Material Job Cost Line')
    labour_job_cost_line_ids = fields.One2many('job.cost.report.line', 'labour_job_cost_report_id', 'Labour Job Cost Line')
    overhead_job_cost_line_ids = fields.One2many('job.cost.report.line', 'overhead_job_cost_report_id',
                                                 'Overhead Job Cost Line')
    tools_job_cost_line_ids = fields.One2many('job.cost.report.line', 'tools_job_cost_report_id', 'Tools Job Cost Line')
    machines_job_cost_line_ids = fields.One2many('job.cost.report.line', 'machines_job_cost_report_id', 'Tools Job Cost Line')
    ...
    total_overhead_cost = fields.Float(compute='_compute_total_overhead_cost', string='Total Overhead Cost',
                                       default=0.0)
    total_tools_cost = fields.Float(compute='_compute_total_tools_cost', string='Total tools Cost',
                                    default=0.0)
    total_machines_cost = fields.Float(compute='_compute_total_machines_cost', string='Total machines Cost',
                                       default=0.0)
    total_cost = fields.Float(compute='_compute_total_cost', string='Total Cost', default=0.0)
    ...
    @api.multi
    def _get_purchase_order_line_count(self):
        for job_cost_report in self:
            purchase_order_line_ids = self.env['purchase.order.line'].search(
                [('job_cost_report_id', '=', job_cost_report.id)])
            job_cost_report.purchase_order_line_count = len(purchase_order_line_ids)
    ...
    def purchase_order_line_button(self):
        ...
        return {
            ...
            'domain': [('job_cost_report_id', '=', self.id)],
        }

    @api.multi
    def _get_invoice_line_count(self):
        for job_cost_report in self:
            invoice_line_ids = self.env['account.invoice.line'].search([('job_cost_report_id', '=', job_cost_report.id)])
            job_cost_report.invoice_line_count = len(invoice_line_ids)
...
class JobCostReportLine(models.Model):
    _name = "job.cost.report.line"

    material_job_cost_report_id = fields.Many2one('job.cost.report', 'Material Job Cost Sheet')
    labour_job_cost_report_id = fields.Many2one('job.cost.report', 'Labour Job Cost Sheet')
    overhead_job_cost_report_id = fields.Many2one('job.cost.report', 'Overhead Job Cost Sheet')
    tools_job_cost_report_id = fields.Many2one('job.cost.report', 'Tools Job Cost Sheet')
    machines_job_cost_report_id = fields.Many2one('job.cost.report', 'Machines Job Cost Sheet')
    item_done = fields.Many2one('item.number', 'Item')
    date = fields.Datetime('Date', default=datetime.now())
    job_type_id = fields.Many2one('job.type', string='Job Type')
    ...
    def _compute_purchase_quantity(self):
        for line in self:
            purchase_order_line_ids = self.env['purchase.order.line'].search([('product_id', '=', line.product_id.id), (
            'account_analytic_id', '=', line.material_job_cost_sheet_id.analytic_ids.id)])
